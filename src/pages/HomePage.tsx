...
const JURISDICTIONS: { name: string; url: string }[] = [
  { name: 'FERPA', url: 'https://en.wikipedia.org/wiki/Family_Educational_Rights_and_Privacy_Act' },
  { name: 'HIPAA', url: 'https://en.wikipedia.org/wiki/Health_Insurance_Portability_and_Accountability_Act' },
  { name: 'FCRA', url: 'https://en.wikipedia.org/wiki/Fair_Credit_Reporting_Act' },
  { name: 'SOX', url: 'https://en.wikipedia.org/wiki/Sarbanes%E2%80%93Oxley_Act' },
  { name: 'GLBA', url: 'https://en.wikipedia.org/wiki/Gramm%E2%80%93Leach%E2%80%93Bliley_Act' },
  { name: 'ADA', url: 'https://en.wikipedia.org/wiki/Americans_with_Disabilities_Act_of_1990' },
  { name: 'GDPR', url: 'https://en.wikipedia.org/wiki/General_Data_Protection_Regulation' },
  { name: 'UK GDPR', url: 'https://en.wikipedia.org/wiki/Data_Protection_Act_2018' },
  { name: 'Kenya DPA', url: 'https://en.wikipedia.org/wiki/Data_Protection_Act,_2019_(Kenya)' },
  { name: 'Australia APP', url: 'https://www.oaic.gov.au/privacy/australian-privacy-principles' },
  { name: 'PIPEDA (Canada)', url: 'https://en.wikipedia.org/wiki/Personal_Information_Protection_and_Electronic_Documents_Act' },
  { name: 'PDPA (Singapore)', url: 'https://en.wikipedia.org/wiki/Personal_Data_Protection_Act_2012_(Singapore)' },
  { name: 'APPI (Japan)', url: 'https://en.wikipedia.org/wiki/Act_on_the_Protection_of_Personal_Information' },
  { name: 'DPDP (India)', url: 'https://en.wikipedia.org/wiki/Digital_Personal_Data_Protection_Act,_2023' },
  { name: 'POPIA (South Africa)', url: 'https://en.wikipedia.org/wiki/Protection_of_Personal_Information_Act,_2013' },
  { name: 'NDPR (Nigeria)', url: 'https://en.wikipedia.org/wiki/Nigeria_Data_Protection_Regulation' },
  { name: 'Law 1581 (Colombia)', url: 'https://en.wikipedia.org/wiki/Law_1581_of_2012' },
  { name: 'PDPA (Thailand)', url: 'https://en.wikipedia.org/wiki/Personal_Data_Protection_Act_(Thailand)' },
];
    ...
    <div ref={pageRef}>
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="grid items-center gap-12 lg:grid-cols-2">
            <div className="text-center lg:text-left">
              ...
              <p className="mb-10 max-w-lg text-lg text-white/70 opacity-0 animate-fade-up md:text-xl" style={{ animationDelay: '0.35s' }}>
      ...
      <Section className="px-6 py-20">
        <div className="mx-auto max-w-5xl">
          <div className="mb-10 text-center">
            ...
            <p className="mx-auto max-w-2xl text-white/70">Multi-jurisdiction operators — US + EU, EU + LATAM, APAC + Africa — get a single posture view instead of one tool per regime. Our roadmap covers the frameworks below, with new regimes added as our customer footprint expands.</p>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {JURISDICTIONS.map((j) => (
                <a
                  ...
                  aria-label={`Learn more about ${j.name}`}
                  className="rounded-sm border border-cyber-cyan/15 bg-cyber-cyan/[0.03] px-3 py-2.5 text-center text-[11px] font-semibold uppercase tracking-wider text-cyber-cyan/70 transition-colors hover:border-cyber-cyan/40 hover:bg-cyber-cyan/10 hover:text-cyber-cyan focus:outline-none focus:ring-2 focus:ring-cyber-cyan/40"
                >
                  {j.name}
                </a>
              ))}
      ...
      <Section id="use-cases" className="px-6 py-24 md:py-32">
        <div className="mx-auto max-w-5xl">
          <div className="mb-16 text-center">
            ...
            <p className="mx-auto max-w-2xl text-white/70">From GRC leads closing the quarter to multi-jurisdiction operators managing 14 regimes at once. See how <Link to="/research/real-cost-of-audit-verification" className="text-cyber-cyan hover:text-white transition-colors underline underline-offset-2">audit verification costs are changing</Link>.</p>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {USE_CASES.map((uc) => (
              <CyberCard key={uc.title} className="p-6 text-center">
                ...
                <p className="text-sm text-white/70">{uc.description}</p>
              </CyberCard>
            ))}
          </div>
        </div>
      ...
      <Section className="px-6 py-24 md:py-32">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {[
              { name: 'Carson Seeger', title: 'Founder & CEO', bio: '10+ years in technical product and program management across regulated industries. Built Arkova to give organizations verification infrastructure that outlasts any single vendor.', photo: '/team-carson.webp', profileUrl: 'https://www.linkedin.com/in/carson-s-8b41061a/', profileType: 'linkedin' as const },
              { name: 'Sarah Rushton', title: 'Founder & COO', bio: 'Over 20 years launching products through compliance-heavy supply chains taught Sarah one thing: documentation verification is broken at every level.', photo: '/team-sarah.webp', profileUrl: 'https://www.linkedin.com/in/sljrushton/', profileType: 'linkedin' as const },
              { name: 'Dr. Yaacov Petscher', title: 'Founder & Advisor', bio: '20 years Research & Data Science experience. Senior Member of the National Academy of Inventors.', photo: '/team-yaacov.webp', profileUrl: 'https://scholar.google.com/citations?user=MUGWLDoAAAAJ&hl=en', profileType: 'scholar' as const },
            ].map((member) => (
              <CyberCard key={member.name} className="p-6 text-center">
                <div className="mx-auto mb-5 h-28 w-28 overflow-hidden rounded-full border border-cyber-cyan/20">
                  <img src={member.photo} alt={`${member.name}, ${member.title} at Arkova`} width={112} height={112} loading="lazy" decoding="async" className="h-full w-full object-cover" />
                </div>
      ...
      <section id="early-access" className="relative overflow-hidden px-6 py-24 md:py-32">
        ...
        <div className="relative mx-auto max-w-2xl text-center">
          ...
          <p className="mb-10 text-lg text-white/70">Join the waitlist. We'll notify you when it's your turn for a pilot. Have questions? <Link to="/contact" className="text-cyber-cyan hover:text-white transition-colors underline underline-offset-2">Contact us</Link>.</p>
